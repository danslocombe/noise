...
fn get_number(obj : &Object, field : &str) -> Result<u64, Error> {
    ...
    raw.as_u64()
        .ok_or(error_simple(format!("'{}' is not a positive integer", field).as_str()))
}
...
fn load_from(ts : &TextureSettings, count : usize, path : &str) -> Result<Vec<Texture>, Error> {
    let mut r = Vec::new();
    for i in 1..count + 1 {
        let path_i = 
            if i < 10 {format!("{}0{}.png", path, i)}
            else      {format!("{}{}.png", path, i)};
        let err = error_simple(format!("could not load file {}", path_i.as_str()).as_str());
        let t : Texture = Texture::from_path_settings(path_i, ts).map_err(|_|{err})?;
        r.push(t);
    }
    Ok(r)
}
...
impl PlayerSpriteManager {
    pub fn new(json_path : &str) -> Result<Self, Error> {
        let mut f = (File::open(json_path)).map_err(|_|{
            error_simple(format!("could not open json file {}", json_path).as_str())})?;
        ...
        let running = load_from(&ts, running_frames as usize, running_path.as_str())?;
        let jumping = load_from(&ts, jumping_frames as usize, jumping_path.as_str())?;
        let swinging = load_from(&ts, swinging_frames as usize, swinging_path.as_str())?;
        let dashing = load_from(&ts, dashing_frames as usize, dashing_path.as_str())?;
